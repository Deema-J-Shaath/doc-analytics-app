...
class MultiLevelClassifier:
    ...
    def load_training_data(self, path='training_data.json'):
        # Adjust path to look in the /home/ubuntu/upload directory
        full_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), '..', path)
        try:
            with open(full_path, 'r', encoding='utf-8') as f:
                self.training_data = json.load(f)
        except FileNotFoundError:
            print(f"[WARN] Training data not found at '{full_path}'")
            self.training_data = []
    ...
    def train(self):
        ...
        if not texts:
            print("[WARN] No valid training data found after filtering. Classifier will not be trained.")
            return
